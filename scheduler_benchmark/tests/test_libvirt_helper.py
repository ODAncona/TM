...
def test_node_config(unique_name):
    ...
    return NodeConfig(
        ...
        network=NetworkConfig(
            name=LIBVIRT_TEST_NETWORK, cidr="192.168.122.0/24"
        ),
        user=UserConfig(name="testuser", sudo=True),
        image="ubuntu",
        disk_size_gb=10,
    )
...
def test_connection(libvirt_connection):
    ...
    assert (
        hostname
    ), f"Impossible d'obtenir le hostname de l'hôte libvirt: {hostname}"

    hypervisor = libvirt_connection.conn.getType()
    assert (
        hypervisor
    ), f"Impossible d'obtenir le type d'hyperviseur: {hypervisor}"
...
def test_create_and_delete_volume(libvirt_connection, unique_name):
    ...
    try:
        ...
        pool = libvirt_connection.conn.storagePoolLookupByName(
            LIBVIRT_POOL_NAME
        )
        for i in range(10):  # Try up to 10 times with 1-second delay
            try:
                ...
                if i == 9:
                    pytest.fail(
                        f"Le volume {volume_name} n'a pas été créé correctement after waiting: {e}"
                    )
                time.sleep(1)
...
def test_create_and_delete_vm(libvirt_connection, test_node_config):
    ...
    try:
        ...
        # Tester delete_vm
        assert (
            libvirt_connection.delete_vm(
                test_node_config.name, delete_storage=True
            )
            is True
        )
        ...
        try:
            ...
            pytest.fail(
                f"Le volume {volume_name} n'a pas été supprimé correctement"
            )
...
def test_context_manager(unique_name):
    ...
    with LibvirtConnection(
        LIBVIRT_HOST, LIBVIRT_USER, LIBVIRT_IDENTITY
    ) as conn:
...
def test_create_volume_from_base_image(libvirt_connection, unique_name):
    ...
    try:
        ...
        try:
            ...
            pytest.skip(
                f"L'image de base {LIBVIRT_BASE_IMAGE} n'existe pas, test ignoré"
            )

        # Créer le volume à partir de l'image de base
        volume_path = libvirt_connection.create_volume(
            volume_name, volume_size, base_image=LIBVIRT_BASE_IMAGE
        )
